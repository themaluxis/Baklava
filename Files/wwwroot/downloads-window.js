...
(function() {
    ...
    async function openDownloadModal(download) {
        console.log('[Downloads] Opening modal for download:', download);
        
        // For completed downloads, use the details-modal.js system
        if (download.status === 'completed' && download.jellyfinId) {
            try {
                // Fetch Jellyfin item details
                const userId = window.ApiClient.getCurrentUserId();
                const item = await window.ApiClient.getItem(userId, download.jellyfinId);
                
                console.log('[Downloads] Fetched Jellyfin item:', item);
                
                // Dispatch custom event to open details modal
                const event = new CustomEvent('openDetailsModal', {
                    detail: {
                        item: {
                            Name: item.Name || download.title,
                            ProductionYear: item.ProductionYear || download.year,
                            jellyfinId: download.jellyfinId,
                            tmdbId: item.ProviderIds?.Tmdb,
                            imdbId: item.ProviderIds?.Imdb,
                            itemType: download.itemType === 'Episode' ? 'series' : 'movie'
                        },
                        isRequestMode: false,
                        isDownloadMode: true,
                ...
                // Wait a moment for modal to open, then inject custom buttons
                setTimeout(() => {
                    injectDownloadButtons(download);
                }, 300);
                
                return;
            } catch (err) {
                console.error('[Downloads] Failed to fetch item, falling back to simple modal:', err);
            }
        }
        
        // For active/queued/failed downloads, show simple status modal
        showSimpleDownloadModal(download);
    }
    
    function injectDownloadButtons(download) {
        const modal = document.querySelector('#item-detail-modal-overlay');
        if (!modal) return;
        
        // Hide request/import/open buttons
        const importBtn = modal.querySelector('#item-detail-import');
        const requestBtn = modal.querySelector('#item-detail-request');
        const openBtn = modal.querySelector('#item-detail-open');
        ...
        if (modal.querySelector('.play-download-btn')) return;
        
        // Find button container (same as import/request buttons)
        const buttonContainer = modal.querySelector('.item-detail-buttons') || 
                               modal.querySelector('#item-detail-import')?.parentElement;
        
        if (!buttonContainer) return;
        
        // Create Play button
        const playBtn = document.createElement('button');
        playBtn.className = 'play-download-btn';
        ...
        deleteBtn.addEventListener('click', async () => {
            ...
            deleteBtn.querySelector('span:last-child').textContent = 'Deleting...';
            ...
            try {
                ...
                console.error('[Downloads] Failed to delete:', err);
                deleteBtn.disabled = false;
                deleteBtn.querySelector('span:last-child').textContent = 'Delete Download';
        ...
        // Insert buttons before import button (or at start of container)
        if (importBtn) {
            buttonContainer.insertBefore(playBtn, importBtn);
            buttonContainer.insertBefore(deleteBtn, importBtn);
        } else {
    ...
    function showSimpleDownloadModal(download) {
        ...
        const progressHtml = download.status === 'active' ? `
            <div style="margin-bottom: 20px;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span style="color: #999; font-size: 14px;">Progress</span>
                    <span style="color: #fff; font-size: 14px; font-weight: bold;">${Math.round(download.progress || 0)}%</span>
                </div>
                <div style="width: 100%; height: 8px; background: rgba(255,255,255,0.1); border-radius: 4px; overflow: hidden;">
                    <div style="height: 100%; background: linear-gradient(90deg, #00a4dc, #0080ff); width: ${download.progress || 0}%; transition: width 0.5s ease;"></div>
                </div>
            </div>
        ` : '';
        
        modal.innerHTML = `
            <div style="background: #181818; border-radius: 8px; width: min(500px, 90vw); padding: 24px; box-shadow: 0 10px 40px rgba(0,0,0,0.8);">
                <h2 style="margin: 0 0 16px 0; color: #fff;">${download.title}</h2>
                ${download.year ? `<div style="color: #999; margin-bottom: 16px;">${download.year}</div>` : ''}
                
                <div style="display: inline-block; padding: 6px 12px; border-radius: 16px; font-size: 12px; font-weight: bold; text-transform: uppercase; margin-bottom: 16px;
                ...
                <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 24px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.1);">
                    ${download.status === 'active' ? '<button class="cancel-btn" style="background: #f44336; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer;">Cancel</button>' : ''}
                    <button class="close-btn" style="background: #555; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer;">Close</button>
                </div>
            </div>
        `;
    ...
    async function init() {
        ...
        if (!enabled) {
            console.log('[Downloads] Disabled in config, skipping initialization');
            return;
        }
        
        console.log('[Downloads] Enabled, initializing...');
        checkAdmin();
